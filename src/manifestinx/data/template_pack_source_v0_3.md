...
**T04** — You commit before constraints stabilize. Fast commitment doesn’t prevent rework.
**T05** — You substitute meta-analysis for bounded definition. A tight frame forces signal to resolve.
**T06** — You treat preference as evidence; signal becomes self-confirming. Without counter-signal, drift persists.
**T07** — You create a new axis when the current one binds. Adding dimensions doesn’t increase truth.
**T08** — You hold incompatible premises in the same frame. The frame fractures under contradiction load.
**T09** — You delay by shifting the task boundary. A fixed boundary exposes avoidance.
**T10** — You diverge interpretations under one label. Coherence degrades when one label carries multiple interpretations.
**T11** — You repeat analysis cycles to avoid committing to a bounded definition. The loop persists because reference never becomes final.
